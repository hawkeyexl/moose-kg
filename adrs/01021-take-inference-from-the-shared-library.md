...
1. **`fill` now retries once.** It previously threw on the first schema-invalid response, failing
   ...
   validation uses the wider configured-field schema, because a provider that volunteers extra
...
survived — it fails if the request schema is used for validation.
