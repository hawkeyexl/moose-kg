...
sources are gone, vendor documentation portals, exported help centers, generated API references.
Nobody can answer "what points at this page before I move it" for those corpora. There is no
source repository to grep.
...
- Markdown has no anchor syntax, so a section's slug can only be derived from its title. HTML
  carries explicit ids, and a link elsewhere in the corpus targets *the id* rather than a slug.
- Markdown headings contain only their text. Real HTML headings contain a permalink widget.
  Sphinx appends `<a class="headerlink" href="#install">¶</a>`, and MkDocs and Docusaurus do the
...
- dockg reads structure rather than semantics. No rule may require knowing a page came from
  Sphinx.
- ADR 01022's asymmetry stands. A missing edge is silent and an extra edge is visible. But a
...
Anchor. **(1a)** always slug the title, as Markdown does. **(1b)** the heading's own `id`,
falling back to a slug. **(1c)** the heading's own `id`, then an enclosing sectioning element's
`id`, then a slug.

`href`. **(2a)** any element, matching ADR 01022. **(2b)** hyperlink elements only.

Heading text. **(3a)** `textContent` verbatim. **(3b)** drop a link from the heading back to its
own anchor.

## Decision Outcome

Options 1c, 2b and 3b win.

**1c. An explicit id beats a slugged title, and a sectioning ancestor's id counts.** Sphinx emits
`<section id="install-the-sdk"><h1>Install the SDK…`, putting the anchor on the section rather
than the heading. Docusaurus and MkDocs put it on the heading. Both are common enough that
supporting only one would leave half of real corpora minting section IRIs that nothing points at.
The ancestor's id is claimed by the *first* heading inside it only. A later sibling heading is not
that section's title, and letting it claim the same id would collapse two sections onto one IRI.
Explicit ids still pass through the same disambiguator as slugs, because nothing guarantees a
document's ids are unique.
...
section edge to a document edge. `stats` reported nothing broken, because nothing *was* broken,
only imprecise. Re-slugging the id reintroduces exactly the failure that preferring the id over
the title was meant to remove.

The exception is IRI safety. `mintSectionIri` does not percent-encode, having always relied on
...
DITA and AsciiDoc actually put in an `id`. Anything stranger, such as a space or a quote or
non-ASCII, still falls back to slugging. Losing the match is the lesser harm against emitting
...
because a spurious edge from an unusual `href` is *occasional*, visible, and correctable. In HTML
it would not be occasional. `<link rel="stylesheet" href="./theme.css">` and
`<link rel="canonical">` sit in the head of every single page. Reading `href` from any element
would therefore add a broken-link finding to every document in the corpus. That is systematic
noise, and it makes the broken-link gate useless, the outcome ADR 01033 was written to prevent.
The
distinction is HTML's own. `<a>` and `<area>` are hyperlink elements, while `<link>` and `<base>`
declare resource relationships. Element identity is knowable here, so it is used.

`src` follows ADR 01022 unchanged. It is an image only on `<img>`, never on `iframe`, `video` or
`script`.
...
information for a reader or a graph, whatever glyph it uses. It matches no ordinary link, because
an ordinary link in a heading points somewhere else. The test suite pins that case too.
...
slice up *by that title*. The slicer once resolved permalinks from the heading's own id while the
analyzer used the inherited one. Every Sphinx-shaped section then indexed no text at all,
...
- **`DEFAULT_LINK_EXTENSIONS` changes**, from `[".md", ".mdx"]` to every readable extension in
  ...
  to a *static asset*. An HTML corpus would therefore have reported none of its broken links.
  Markdown stays ahead of HTML in the order. A corpus holding both a source and its build output
  therefore resolves a pretty URL to the source.
...
- Bad. `dockg fill --apply` refuses HTML. docmeta's HTML extractor exposes `apply`, so this is a
  dockg limitation with a clear path out rather than a property of the format.
- Known gap. `<img srcset>` and `<picture><source>` are not read. Under ADR 01022's rule, `src`
  on a `<source>` element is not necessarily an image. A missing image is a visible absence
  rather than a wrong type assertion.

### Confirmation

`test/unit/analyze-html.test.ts` covers the derivation rules case by case. That includes the
negatives that would otherwise pass for the wrong reason. `<link rel="canonical">` produces no
edge, `<iframe src>` produces no image, and a real link inside a heading survives while the
permalink is dropped. `test/unit/html-text.test.ts` covers the slicer, opening with the
inherited-anchor regression.

`test/fixtures/formats/mixed/` is an HTML-and-Markdown corpus with its own two goldens.
`test/integration/formats-mixed.test.ts` holds it to the same gates as the main corpus. Those
are double-build byte comparison, version-normalized golden and n3 round-trip. It adds the
cross-format link assertions and `dockg check`. It also covers the search-index golden with a
blunt "no markup" backstop, the per-format iiRDS media type, and the absence of any timestamp.
...
- Bad. Mints section IRIs nothing links to. Every anchor link in an HTML corpus targets an id,
  so `traverse` and `--reverse` would miss all of them. That is the ADR 01022 failure again.
...
- Good. Structural, tool-agnostic, and unable to misfire. A heading linking to itself has no
  content to lose.
