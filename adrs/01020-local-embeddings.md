...
Phase 8 gave moose-kg lexical entry: a text query reaches nodes by keyword. It
...
At moose-kg's scale the trade is plainly bad: int8 would shrink a once-fetched,
...
- **Orama was evaluated seriously and rejected on evidence.** It runs
  ...
  moose-kg's "no wall clock anywhere" invariant; its hybrid sort has **no
  ...
  internally — the same loop moose-kg would write — at +18.6 KB gzipped.
...
the recipe cannot drift from the one `moose-kg embed` records. Omitted, that half
simply does not run; the README says so rather than promising a check the code
has no way to perform.

### 5. `moose-kg/embed` is an opt-in subpath, never a dependency
...
measurement) — making every moose-kg user carry that for a feature most will not
enable is indefensible. `moose-kg embed` without it exits 2 naming the install
command. Behind its own subpath entry, a consumer who never imports
`moose-kg/embed` never resolves it, sidestepping the unresolved
...
load-bearing. Keeping fusion in moose-kg's own code is what makes it deterministic
...
- Good: build and query vectors provably come from the same function.
...
→ `MooseKgError`, L2 normalization), for standalone vector search (ranking, IRI
...
A documented, non-automated real-model smoke verifies Node and browser produce
matching top-k — the claim the whole design rests on.
