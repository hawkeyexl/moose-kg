...
changes what the advice has to say without changing who is receiving it, so folding it into
either segment would lose it, and giving it its own persona would duplicate one that exists.
...
**The threshold is a ratchet, not a target.** The useful pattern is to set the coverage
threshold just below current coverage so the gate holds the line, then raise it as the backlog
shrinks. Setting it at the eventual goal on day one makes the build permanently red, which
...
[`cuj-prove-coverage`](../journeys/prove-coverage.md) — both of which otherwise read as though
the corpus starts healthy.
