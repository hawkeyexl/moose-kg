...
something*. It is the payoff for the work the rest of them do.

It is reached two ways. Priya arrives from `stats`, which answers aggregate questions ("how many
orphans?") and immediately provokes specific ones ("which pages point at *this* one?"). Ines
arrives from [`cuj-model-concepts`](model-concepts.md) needing to verify the vocabulary landed as
written. Slug convergence in particular is silent, and a query is the only way to see it.

## What they need to reach, in order

1. **Which command answers which shape of question.** `query` matches a triple pattern with any
   term omitted as a wildcard, which suits "show me every X". `traverse` walks edges from a node,
   which suits "what is connected to this". Readers reach for the wrong one first, and the
   distinction costs one sentence.
...
4. **Read the exclusions.** A scoped traversal prints what it removed and why. A reader who
   checks only the returned nodes cannot tell a working filter from one that matched nothing.
   That is the same open-world hazard as [`cuj-scope-by-variant`](scope-by-variant.md), one
   layer up.
...
- **Show real output.** Both commands print a compact, stable format. A reader matching their
