...
trigger: >-
  ...
  corpus links by site route, and moose-kg resolved them against the filesystem
entry_point: /moose-kg/build/routes/
success_criteria: >-
...
Priya's corpus links the way the site publishes, and moose-kg resolved those links against the
filesystem instead.
...
track, and that section has to hand off clearly or the reader concludes moose-kg's link detection
is simply wrong.
...
`src/content/docs/actions/find.mdx`. moose-kg does not know that until it is told. Once it is,
...
1. **The diagnosis.** A broken-link report whose entries look correct on the live site means
   the corpus links by route and moose-kg is resolving by path. Naming that pattern is most of the
   fix.
