...
import { MOOSE_KG, NS, ROLE } from "../../src/core/vocab.js";
...
describe("deriveGraph — document basics", () => {
  it("types every doc and records its path", () => {
    ...
    expect(has(g, DOC, `${NS.rdf}type`, iri(`${MOOSE_KG}Document`))).toBe(true);
    expect(has(g, DOC, `${MOOSE_KG}path`, lit("docs/a.md"))).toBe(true);
  });
...
describe("deriveGraph — provenance", () => {
  ...
  it("maps kg.derivedFrom to resolved docs, URLs, and broken links", () => {
    ...
    expect(has(g, DOC, `${MOOSE_KG}brokenLink`, lit("missing.md"))).toBe(true);
  });
  ...
  it("keeps a '## Generation' heading distinct from the generation activity", () => {
    ...
    const activity = `${DOC}#prov.generation`;
    expect(has(g, section, `${NS.rdf}type`, iri(`${MOOSE_KG}Section`))).toBe(
      true,
    );
    ...
    expect(has(g, activity, `${NS.rdf}type`, iri(`${MOOSE_KG}Section`))).toBe(
      false,
    );
  });
  ...
  it("maps kg.provenance entries to per-model fill activities, not shared subjects", () => {
    ...
    const claudeEntry = `${claudeActivity}.field.prefLabel`;
    expect(
      has(g, claudeActivity, `${MOOSE_KG}filledFieldEntry`, iri(claudeEntry)),
    ).toBe(true);
    expect(
      has(g, claudeEntry, `${MOOSE_KG}filledField`, lit("prefLabel")),
    ).toBe(true);
    ...
    const gptEntry = `${gptActivity}.field.subjects`;
    expect(has(g, gptEntry, `${MOOSE_KG}filledField`, lit("subjects"))).toBe(
      true,
    );
  ...
  it("still accepts the legacy single-object kg.provenance form", () => {
    ...
    const entry = `${activity}.field.subjects`;
    expect(has(g, activity, `${MOOSE_KG}filledFieldEntry`, iri(entry))).toBe(
      true,
    );
    expect(has(g, entry, `${MOOSE_KG}filledField`, lit("subjects"))).toBe(true);
  });
  ...
  it("emits per-field confidence as a decimal on the fill-field entry (ADR 01015)", () => {
    ...
    const entry = `${DOC}#prov.kg-fill.m1.field.prefLabel`;
    expect(
      has(g, entry, `${MOOSE_KG}confidence`, lit("0.9", `${NS.xsd}decimal`)),
    ).toBe(true);
  });
  ...
  it("emits the graph-level build activity with tool agent and prov:used edges", () => {
    ...
    const tool = `${BASE}agent/software/moose-kg`;
    ...
    expect(has(g, tool, `${MOOSE_KG}version`, lit("1.2.3"))).toBe(true);
  });
  ...
  it("maps declared kg.revisionOf like derivedFrom: resolved, URL, or broken", () => {
    ...
    expect(has(g, DOC, `${MOOSE_KG}brokenLink`, lit("gone.md"))).toBe(true);
  });
...
describe("deriveGraph — qualified provenance", () => {
  ...
  it("qualifies generation and build associations", () => {
    ...
    const buildAssoc = `${BASE}activity/build.assoc.moose-kg`;
    expect(
      has(
        g,
        `${BASE}activity/build`,
...
describe("deriveGraph — sections", () => {
  it("nests sections under doc and parent sections with level and order", () => {
    ...
    const secA = `${DOC}#a`;
    const secB = `${DOC}#b`;
    expect(has(g, secA, `${NS.rdf}type`, iri(`${MOOSE_KG}Section`))).toBe(true);
    expect(has(g, DOC, `${NS.dcterms}hasPart`, iri(secA))).toBe(true);
    expect(has(g, secA, `${NS.dcterms}hasPart`, iri(secB))).toBe(true);
    expect(has(g, secB, `${MOOSE_KG}level`, lit("2", `${NS.xsd}integer`))).toBe(
      true,
    );
    expect(has(g, secB, `${MOOSE_KG}order`, lit("1", `${NS.xsd}integer`))).toBe(
      true,
    );
  });
});
...
describe("deriveGraph — negative scope (ADR 01014)", () => {
  it("emits notApplicableToVariant + a ProductVariant node, and notSoftwareSubject", () => {
    ...
    expect(has(g, DOC, `${MOOSE_KG}notApplicableToVariant`, iri(v))).toBe(true);
    expect(has(g, v, `${NS.rdf}type`, iri(`${NS.iirds}ProductVariant`))).toBe(
      true,
    );
  ...
  it("attaches negative scope to a section too", () => {
    ...
    expect(
      has(
        g,
        SEC,
        `${MOOSE_KG}notApplicableToVariant`,
  ...
  it("emits no negative-scope triples when the fields are absent", () => {
    const g = graph({ "docs/a.md": "---\nkg:\n  topicType: task\n---\n" });
    expect(g.some((q) => q.p === `${MOOSE_KG}notApplicableToVariant`)).toBe(
      false,
    );
    expect(g.some((q) => q.p === `${MOOSE_KG}notSoftwareSubject`)).toBe(false);
  });
});
...
describe("deriveGraph — section-level metadata (ADR 01013)", () => {
  ...
  it("emits moose-kg:brokenSectionRef for a key naming no heading", () => {
    const g = graph({
      "docs/a.md":
        "---\nkg:\n  sections:\n    nope:\n      topicType: task\n---\n\n# A\n\n## Install\n",
    });
    expect(has(g, DOC, `${MOOSE_KG}brokenSectionRef`, lit("nope"))).toBe(true);
  ...
  it("emits no section metadata when kg.sections is absent", () => {
    const g = graph({ "docs/a.md": "# A\n\n## Install\n" });
    expect(g.some((q) => q.p === `${MOOSE_KG}brokenSectionRef`)).toBe(false);
...
describe("deriveGraph — images, code, derive toggles", () => {
  it("maps images and code languages", () => {
    ...
    expect(has(g, DOC, `${MOOSE_KG}codeLanguage`, lit("python"))).toBe(true);
  });
