...
describe("HTML section text", () => {
  it("finds a section whose anchor was inherited from its <section>", async () => {
    ...
    const doc = await analyzeDoc(SPHINX, "docs/a.html", NO_PATHS);
    const title = doc.sections[0]!.title;
    expect(title).toBe("Install the SDK");
    expect((await textOf(SPHINX)).sectionOwnText(title, 1, 0)).toBe(
      "Install the SDK\n\nStart here.",
    );
  });

  it("gives a section only its own text, not its subsections'", async () => {
    const text = await textOf(SPHINX);
  ...
  it("gives the document only the prose no section owns", async () => {
    expect((await textOf(SPHINX)).preamble()).toBe("Preamble prose.");
  });

  it("recovers prose, never markup", async () => {
    const body = (await textOf(SPHINX)).body;
    expect(body).not.toMatch(/[<>]/);
    expect(body).toContain("Install the SDK");
  });

  it("keeps block-level elements on separate lines", async () => {
  ...
  it("omits scripts and styles", async () => {
    const text = await textOf(
  ...
  it("disambiguates repeated headings by occurrence", async () => {
    const text = await textOf(
  ...
  it("returns undefined for a heading that is not there", async () => {
    expect(
      (await textOf(SPHINX)).sectionOwnText("Nonexistent", 2, 0),
    ).toBeUndefined();
  });
});
